...
//Set weather
  window.addEventListener('load', ()=> {
    ...
    let humidi = document.querySelector('.humidity');
    let longit = document.querySelector('.longitude');
    let latti = document.querySelector('.lattitude');
    //tommorow
    let temperatureTomorrow = document.querySelector('.temperature-tomorrow-degree');
   
    //tommorow1
    let temperatureTomorrow1 = document.querySelector('.temperature-tomorrow-degree1');
    //tommorow2
    let temperatureTomorrow2 = document.querySelector('.temperature-tomorrow-degree2');

    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(position => {
        long = position.coords.longitude;
        lat = position.coords.latitude;
        var api = `https://api.darksky.net/forecast/5d8e47a0af37f840684d8c0e0564a77c/${lat},${long}?units=si&lang=en`;
        ...
        .then(data => {
          const {temperature, summary, icon, apparentTemperature, windSpeed, humidity} = data.currently;
          //language count
      ...
      function setIcons(icon, iconID) {
        const skycons = new Skycons({color: "rgb(234, 171, 240)"});
...
function searchWeather(searchTerm) {
    ...
    var proxyUrl = 'https://cors-anywhere.herokuapp.com/';
    fetch(proxyUrl + `http://api.openweathermap.org/data/2.5/weather?${saerchMethod}=${searchTerm}&APPID=${appKey}&units=${units}`)
    .then(result => {
        return result.json();
    }).then(resulti => {
        init(resulti);
...
function init(resultfromserver) {
    let long = resultfromserver.coord.lon;
    ...
    .then(data => {
      ...
      const {temperature, summary, apparentTemperature, windSpeed, humidity} = data.currently;
      //tommorow
      let temperatureTomorrowHourly = (data.daily.data[1].temperatureMax + data.daily.data[1].temperatureMin)/2;
      //tommorow1
      let temperatureTomorrowHourly1 = (data.daily.data[2].temperatureMax + data.daily.data[2].temperatureMin)/2;
      //tommorow2
      let temperatureTomorrowHourly2 = (data.daily.data[3].temperatureMax + data.daily.data[3].temperatureMin)/2;
      //Set DOM
      temperatureDegree.textContent = Math.ceil(temperature);
      temperatureDescription.textContent = summary;
      apparentTemp.textContent = 'FEELS LIKE: ' + Math.ceil(apparentTemperature);
